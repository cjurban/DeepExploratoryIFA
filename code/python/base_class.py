...
class BaseClass():
    ...
    def __init__(self,
        ...
        Args:
            ...
            monitor_convergence  (Boolean): Whether to monitor convergence on a test set.
        """
        ...
        for batch_idx, data in enumerate(train_loader):
            if not self.converged:
                ...
                if (self.global_iter - 1) % self.log_interval == 0:
                    # Monitor convergence.
                    if self.monitor_convergence:
                        eval_loss = self.test(eval_loader, mc_samples, iw_samples, print_result = False)
                        self.convergence_list.append(eval_loss / len(eval_loader.dataset))
