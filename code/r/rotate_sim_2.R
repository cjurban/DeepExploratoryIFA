...
#
# Purpose: Rotate MH-RM comparison factor loadings.
#
###############################################################################
...
for (cond in 1:4) {
  ...
  dir.create(file.path(base_dir, paste0("results/sim_2/aiwvi/cond_", toString(cond - 1)), "rotated_loadings"), showWarnings = FALSE)
  dir.create(file.path(base_dir, paste0("results/sim_2/aiwvi/cond_", toString(cond - 1)), "covs"), showWarnings = FALSE)
  
  # Rotate factor loadings and save results.
  loadings_filenames = unlist(lapply(1:100, function(i) {file.path(base_dir,
                                                                   paste0("results/sim_2/aiwvi/cond_", toString(cond - 1)),
                                                                   "loadings",
  ...
  lapply(seq_along(loadings_filenames), function(i) {
    ...
    t2 <- Sys.time()
    write.table(rotated[[1]], file.path(base_dir, paste0("results/sim_2/aiwvi/cond_", toString(cond - 1), "/rotated_loadings/rotated_loadings_", i - 1, ".txt")), sep = " ", quote = FALSE, row.names = FALSE, col.names = FALSE)
    write.table(rotated[[2]], file.path(base_dir, paste0("results/sim_2/aiwvi/cond_", toString(cond - 1), "/covs/cov_", i - 1, ".txt")), sep = " ", quote = FALSE, row.names = FALSE, col.names = FALSE)
